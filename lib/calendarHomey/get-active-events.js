      ...
      try {
        ...
        if (event.recurrences !== undefined) {
          for (const r of Object.keys(event.recurrences)) {
            ...
            const rMomentRecurr = event.recurrences[r].recurrenceid.toUTCString().includes('00:00:00') ? moment(event.recurrences[r].recurrenceid) : moment.tz(event.recurrences[r].recurrenceid, usedTZ)
            ...
            const rMomentStart = event.recurrences[r].start.toUTCString().includes('00:00:00') ? moment(event.recurrences[r].start) : moment.tz(event.recurrences[r].start, usedTZ)
            const rDateStart = new Date(event.recurrences[r].start)
            ...
            if (rMomentStart.isBetween(eventLimitStart, eventLimitEnd) === true) {
              ...
              const rMomentEnd = event.recurrences[r].end.toUTCString().includes('00:00:00') ? moment(event.recurrences[r].end) : moment.tz(event.recurrences[r].end, usedTZ)
              if (rMomentStart.isBefore(eventLimitStart) && rMomentEnd.isAfter(eventLimitStart)) {
                const existAlready = dates.filter(date => date === rDateStart)
                if (existAlready.length === 0) {
                  dates.push(rDateStart)
                  console.log(`Recurrence in '${event.uid}' is ongoing from '${rMomentRecurr}' to '${rMomentEnd}'`)
                }
              }
            }
          }
