        ...
        if (end.isBefore(now) || start.isAfter(eventLimitEnd)) {
          ...
          if (newEvent.exdate[dateLookupKey] !== undefined) {
            ...
            if (exdate.isSame(start, 'day')) {
              console.log(`Skipping '${newEvent.summary}' @ `, date, '(', newEvent.exdate[dateLookupKey], ')', 'because it exists in exdate by date lookup key')
              addEvent = false
            }
            ...
            if (exdate.isSame(start, 'day')) {
              console.log(`Skipping '${newEvent.summary}' @ `, date, '(', newEvent.exdate[previousDaysLookupKey], ')', 'because it exists in exdate by previousDay lookup key')
              addEvent = false
            }
          }
        }
