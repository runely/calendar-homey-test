...
const { debug, info, warn, error } = require('../../config') // { info, warn, error, debug }
...
module.exports = async calendarsItem => {
  ...
  try {
    ...
    const d = options.downloadIcs || options.saveAll || options.saveActive ? new Date() : undefined
    ...
    const isPrintEventByIndex = typeof options.printEventByIndex === 'number' && options.printEventByIndex > -1
    const isPrintEventByUIDs = Array.isArray(options.printEventByUIDs) && options.printEventByUIDs.length > 0
    ...
    if (isPrintEventByIndex || isPrintEventByUIDs) {
      ...
      if (isPrintEventByUIDs) {
        ...
        for (const uid of options.printEventByUIDs) {
          const lowerCasedUID = uid.toLowerCase()
    ...
    const stackTrace = typeof error_ === 'object' ? error_.stack : undefined
