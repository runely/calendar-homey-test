 ...
 *
 * Calendar will apply the timezone where the Homey is located (chosen in Homey app -> More -> Settings -> Position) to get the correct time for the event
 *
 * However, if an event uses Customized Time Zone, node-ical will treat this as UTC, and Calendar will apply the timezone from the Homey. This will probably not be correct
 *
 * Therefore, when Calendar finds Africa/Abidjan as start tz and Etc/UTC as DTStamp tz, the event will be flagged as not to have the Homey timezone applied. Meaning that the raw time shown in the ics file is what will be used
 *
 */
...
(async () => {
  ...
  for await (const calendar of calendars) {
    ...
    if (showOnlyTheseUids.length > 0) {
      //const activeEvents: CalendarEvent[] = activeData.filter((ev: CalendarEvent) => showOnlyTheseUids.find((uid: string) => ev.uid.startsWith(uid)));
      const activeEvents: CalendarEvent[] = activeData.filter((ev: CalendarEvent) => showOnlyTheseUids.includes(ev.uid));
